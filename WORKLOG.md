...
## 2026-09-03: 사용 종료 자산 정리 — `off-campus_courses/` · 출석부 뷰어 · `pilot001.html`

담당자 판단으로 역할이 끝난 자산을 걷어냈다. **삭제 전 인바운드 링크를 전수 검사해 3건 모두 0건**임을 확인했으므로 내부 죽은 링크는 없다. 다만 외부에 배포된 URL은 깨진다.

### 1. `off-campus_courses/` 폴더 삭제 (3개 문서)

내용이 `dream_up/`으로 이전되어 더 이상 쓰지 않는다. 원본은 담당자가 별도 백업 보유.

| 삭제 | 제목 |
|---|---|
| `index.html` (약 1,120줄) | 2026학년도 2학기 고교-대학 연계 학점 인정 학교 밖 교육 과목 심의 자료 |
| `dream_up_26-s.html` (약 523줄) | 2026 여름방학 꿈키움 학교 밖 교육 강좌 안내 |
| `dream_up26-1/index.html` (약 258줄) | 2026학년도 1학기 꿈키움 학교 밖 교육 이수 결과 확인 |

> ⚠️ **`safety/off-campus_courses.html`은 별개 파일이며 그대로 있다.** 이름이 같아 혼동하기 쉽다. 이제 폴더는 없고 파일만 남았다.

### 2. 출석부 뷰어 → 열람 종료 안내로 교체

`dream_up/attendance/`의 출석부 이미지 **159장(`pages/`)을 삭제**하고, `index.html`을 안내 페이지로 교체했다.
아직 외부 URL로 들어오는 사람이 있을 수 있어 폴더 자체는 남겼다.

- 스캔본에 **학생 명단이 그대로 들어 있는데 인증 장치 없이 공개**되어 있었다. 다시 게시할 일이 생기면 접근 통제를 먼저 붙일 것.
- 안내 페이지는 `apply/index.html`(마감 안내)의 `.closed-card` 구조를 그대로 따랐다. 공통 헤더·푸터 사용(depth 2 → `../../assets/`), `data-active="safety"`.
- GoatCounter를 **넣었다.** 만료 후에도 유입이 있는지 확인해 폴더를 언제 지울지 판단하기 위해서다.
- 문체는 원칙 5-1에 맞춰 사실만 적었다. 남은 안내는 "소속 학교 담당 교사에게 문의".

### 3. `pilot001.html` 삭제

홈 구 시안. 정식 홈이 `index.html`로 확정된 뒤 역할이 끝나 「정리 판단 보류」에 올려 두었던 항목이다.

### 4. `dream_up/attendance/` README 대소문자 충돌 해소

git에 `README.md`(32줄, 원본)와 `readme.md`(1줄)가 **둘 다 등록**되어 있었다. Windows는 대소문자를 구분하지 않아 나중에 올라온 1줄짜리가 32줄 원본을 디스크에서 덮어쓴 상태였고, 그대로 커밋하면 원본이 소실될 뻔했다.

- `git update-index --force-remove`로 **소문자 항목만 정확히** 인덱스에서 제거 (`git rm`은 `core.ignorecase=true`라 대상이 어긋날 수 있다)
- `git show HEAD:...README.md`로 원본 32줄 복원 + 맨 앞에 뷰어를 내린 사실을 주석으로 추가
- 원본에 담긴 강좌 코드 매칭 방식·전화번호 마스킹 내역은 운영 기록이라 보존했다

> 앞으로 GitHub 웹에서 파일을 올릴 때 **기존 파일과 대소문자만 다른 이름을 쓰지 말 것.** 로컬(Windows)에서 조용히 덮어써진다.
...
- [ ] **`dream_up/attendance/` 폴더 존치 기간** — 열람 종료 안내만 남겨 두었다. GoatCounter로 유입이 끊긴 것을 확인한 뒤 폴더째 삭제할지 결정한다.

> ✅ **`pilot001.html`은 2026-09-03에 삭제**되어 목록에서 내렸다.
